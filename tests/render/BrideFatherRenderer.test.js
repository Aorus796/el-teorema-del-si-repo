...
/*
 * Cubre la cache de sprites de BrideFatherRenderer.js (propia, local a
 * ese módulo -- ver el comentario en el propio archivo). Mismo patrón
 * que tests/render/CorolariaRenderer.test.js/ElenaRenderer.test.js/
 * GonzaloRenderer.test.js: simula un `document` mínimo ANTES de importar
 * el renderer, para que su cache tome la rama "hay DOM" en vez del
 * fallback usado por el resto de la suite. Vive en su propio archivo por
 * la misma razón: la cache es un Map de módulo compartido entre todos
 * los test() de este archivo bajo `node --test`.
 */
...
const { renderBrideFather, BRIDE_FATHER_DIMENSIONS } = await import(
  "../../src/render/BrideFatherRenderer.js"
);

test("BRIDE_FATHER_DIMENSIONS coincide con las dimensiones declaradas en los datos", () => {
  assert.equal(BRIDE_FATHER_DIMENSIONS.width, BRIDE_FATHER_PIXEL_WIDTH);
  assert.equal(BRIDE_FATHER_DIMENSIONS.height, BRIDE_FATHER_PIXEL_HEIGHT);
});

test("renderBrideFather() rasteriza el frontal en un canvas aparte, sin smoothing, y lo reutiliza con drawImage", () => {
  createdCanvases.length = 0;

  const context = new FakeGameContext();
  renderBrideFather(context, 10, 20, "down");

  assert.equal(createdCanvases.length, 1);
  const [canvas] = createdCanvases;

  assert.equal(canvas.width, BRIDE_FATHER_PIXEL_WIDTH);
  assert.equal(canvas.height, BRIDE_FATHER_PIXEL_HEIGHT);
...
test("un segundo render con el mismo facing no crea un canvas nuevo (cache, no reconstrucción por frame)", () => {
  ...
  renderBrideFather(first, 10, 20, "down");
  renderBrideFather(second, 15, 25, "down");
...
test("facing 'up' (espalda) cachea un canvas DISTINTO del frontal", () => {
  ...
  const context = new FakeGameContext();
  renderBrideFather(context, 0, 0, "up");
...
test("facing 'right' (lateral) cachea un tercer canvas distinto", () => {
  ...
  const context = new FakeGameContext();
  renderBrideFather(context, 0, 0, "right");
...
test("facing 'left' aplica una transformación de canvas (save/scale/translate/restore) para reflejar, sin rasterizar de nuevo", () => {
  const context = new FakeGameContext();
  renderBrideFather(context, 0, 0, "left");
