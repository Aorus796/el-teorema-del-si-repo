...
/*
 * Cubre la cache de sprites de SilogioRenderer.js (propia, local a ese
 * módulo -- ver el comentario en el propio archivo). Mismo patrón que
 * tests/render/BrideFatherRenderer.test.js/CorolariaRenderer.test.js/
 * ElenaRenderer.test.js/GonzaloRenderer.test.js: simula un `document`
...
const { renderSilogio, SILOGIO_DIMENSIONS } = await import(
  "../../src/render/SilogioRenderer.js"
);

test("SILOGIO_DIMENSIONS coincide con las dimensiones declaradas en los datos", () => {
  assert.equal(SILOGIO_DIMENSIONS.width, SILOGIO_PIXEL_WIDTH);
  assert.equal(SILOGIO_DIMENSIONS.height, SILOGIO_PIXEL_HEIGHT);
});

test("renderSilogio() rasteriza el frontal en un canvas aparte, sin smoothing, y lo reutiliza con drawImage", () => {
  createdCanvases.length = 0;

  const context = new FakeGameContext();
  renderSilogio(context, 10, 20, "down");

  assert.equal(createdCanvases.length, 1);
  const [canvas] = createdCanvases;

  assert.equal(canvas.width, SILOGIO_PIXEL_WIDTH);
  assert.equal(canvas.height, SILOGIO_PIXEL_HEIGHT);
...
test("un segundo render con el mismo facing no crea un canvas nuevo (cache, no reconstrucción por frame)", () => {
  ...
  renderSilogio(first, 10, 20, "down");
  renderSilogio(second, 15, 25, "down");
...
test("facing 'right' (lateral) cachea un tercer canvas distinto", () => {
  ...
  const context = new FakeGameContext();
  renderSilogio(context, 0, 0, "right");
...
test("facing 'left' aplica una transformación de canvas (save/scale/translate/restore) para reflejar, sin rasterizar de nuevo", () => {
  const context = new FakeGameContext();
  renderSilogio(context, 0, 0, "left");
